...
[APIM alternative CLI runbook](../docs/apim-deployment.md) adds a third entry
point without changing the native or Function templates.
...
| [`apim.bicep`](apim.bicep) | Dedicated `rg-copilot-otel-apim` subscription entry, own marker/receipt |
| [`apim-service.bicep`](apim-service.bicep) | APIM Developer service and system identity |
| [`apim-baseline.bicep`](apim-baseline.bicep) | Subscription-required API with initial deny policy |
| [`apim-operations.bicep`](apim-operations.bicep) | Fixed POST operations and native backend policies |
| [`apim-activate.bicep`](apim-activate.bicep) | Explicit activation policy and generated API-scoped subscription after baseline review |
| [`apim-publisher.bicep`](apim-publisher.bicep) | Additive publisher role on the exact existing native DCR only |

## APIM isolation and authentication

APIM is a parallel gateway, not a Function backend or a replacement monitoring
stack. Its only cross-stack write is its own DCR-scoped publisher assignment;
no native/Function resource or workbook update belongs in this deployment.
The key is scoped to the dedicated telemetry API, generated by APIM and absent
from template parameters/outputs. API-level policy also rejects broad
all-access/product subscriptions. APIM managed identity, not the client key,
authenticates to the exact verified DCE routes.

Developer-tier support is for evaluation, not enterprise production acceptance.
See the runbook for staged fail-closed provisioning, supported bounds and
encoding restrictions, SKU/cost review, key rotation, receipt validation and
fresh positive/negative E2E requirements. Compiled templates and policy text
tests do not execute the managed APIM policy engine.
